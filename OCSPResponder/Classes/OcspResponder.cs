...
namespace OcspResponder.Core
{
    ...
    public class OcspResponder : IOcspResponder
    {
        ...
        public OcspHttpResponse Respond(OcspHttpRequest httpRequest)
        {
            try
            {
                OcspReqResult ocspReqResult = GetOcspRequest(httpRequest);
                ...
                OcspResp ocspResponse = GetOcspDefinitiveResponse(ocspReqResult.OcspRequest, ocspReqResult.IssuerCertificate);
        ...
        /// <returns></returns>
        private OcspResp GetOcspDefinitiveResponse(OcspReq ocspRequest, X509Certificate issuerCertificate)
        {
            
            var basicResponseGenerator = new BasicOcspRespGenerator(new RespID(OcspResponderRepository.GetResponderPublicKey(issuerCertificate)));

            var extensionsGenerator = new X509ExtensionsGenerator();

            var nextUpdate = OcspResponderRepository.GetNextUpdate();
            ...
            foreach (var request in ocspRequest.GetRequestList())
            {
                ...
                CaCompromisedStatus caCompromisedStatus = OcspResponderRepository.IsCaCompromised(issuerCertificate);
                ...
                else
                {
                    ...
                    if (OcspResponderRepository.SerialExists(serialNumber, issuerCertificate))
                    {
                        var status = OcspResponderRepository.SerialIsRevoked(serialNumber, issuerCertificate);
                        certificateStatus = status.IsRevoked ? new RevokedStatus(status.RevokedInfo.Date.UtcDateTime, (int)status.RevokedInfo.Reason) : CertificateStatus.Good;
                    }
                    else
                    {
            ...
            var basicOcspResponse = basicResponseGenerator.Generate(
                signatureAlgorithm,
                OcspResponderRepository.GetResponderPrivateKey(issuerCertificate),
                OcspResponderRepository.GetChain(issuerCertificate),
        ...
        private static void SetNonceExtension(OcspReq ocspRequest, X509ExtensionsGenerator extensionsGenerator)
        {
        ...
        private OcspReqResult GetOcspRequest(OcspHttpRequest httpRequest)
        {
            ...
            var issuerCerts = OcspResponderRepository.GetIssuerCertificates();
        ...
        private static OcspReq CreateOcspReqFromHttpRequest(OcspHttpRequest httpRequest)
        {
            return httpRequest.HttpMethod.ToUpper() switch
            {
                "GET" => CreateOcspReqFromGet(httpRequest),
                "POST" => CreateOcspReqFromPost(httpRequest),
                _ => throw new HttpRequestException("Only GET and POST methods are allowed"),
            };
        }
        ...
        /// <param name="httpRequest"><see cref="OcspHttpRequest"/></param>
        /// <returns><see cref="OcspReq"/></returns>
        private static OcspReq CreateOcspReqFromPost(OcspHttpRequest httpRequest)
        {
        ...
        /// <param name="httpRequest"><see cref="OcspHttpRequest"/></param>
        /// <returns><see cref="OcspReq"/></returns>
        private static OcspReq CreateOcspReqFromGet(OcspHttpRequest httpRequest)
        {
        ...
        private static OcspHttpResponse CreateResponse(byte[] ocspResponseBytes)
        {
